  ...
  describe('initialising the component', () => {
    ...
    describe('without existing analytics', () => {
      it('does not hide the warning message', () => {
        ...
        expect(document.querySelector('.pay-cookie-settings-alert--warning').style.display).not.toEqual('none')
      })
    })
    ...
    describe('with existing analytics', () => {
      it('hides the warning message', () => {
        ...
        expect(document.querySelector('.pay-cookie-settings-alert--warning').style.display).toEqual('none')
      })
    })
    ...
    describe('initialising the Cookie Settings radio buttons', () => {
      it('when no cookie present - no radio buttons are checked', () => {
        ...
        expect(document.querySelector('input[name=cookies-analytics][value=on]').checked).toEqual(false)

        expect(document.querySelector('input[name=cookies-analytics][value=off]').checked).toEqual(false)
      })
      ...
      it('cookie with analytics=true - ON radio button is checked', () => {
        ...
        expect(document.querySelector('input[name=cookies-analytics][value=on]').checked).toEqual(true)
      ...
      it('cookie with analytics=false - OFF radio button is checked', () => {
        ...
        expect(document.querySelector('input[name=cookies-analytics][value=off]').checked).toEqual(true)
      })
    })
  })
  ...
  describe('Submitting the form', () => {
    describe('with No selected', () => {
      ...
      it('does not initialise Analytics', () => {
        window.GovUkPay.CookieSettings.submitSettingsForm(submitEvent)
        expect(window.GovUkPay.InitAnalytics.InitialiseAnalytics).not.toHaveBeenCalled()
      })
      ...
      it('shows confirmation message and hides other messages', () => {
        ...
        expect(document.querySelector('.pay-cookie-settings-alert--warning').style.display).toEqual('none')
        expect(document.querySelector('.pay-cookie-settings-alert--error').style.display).toEqual('none')
        expect(document.querySelector('.pay-cookie-settings-alert--success').style.display).toEqual('block')
      })
    })
    ...
    describe('with Yes selected', () => {
      ...
      it('initialises Analytics', () => {
        window.GovUkPay.CookieSettings.submitSettingsForm(submitEvent)
        expect(window.GovUkPay.InitAnalytics.InitialiseAnalytics).toHaveBeenCalled()
      })
    ...
    describe('with nothing selected', () => {
      ...
      it('shows error message', () => {
        ...
        expect(document.querySelector('.pay-cookie-settings-alert--warning').style.display).not.toEqual('none')
        expect(document.querySelector('.pay-cookie-settings-alert--error').style.display).toEqual('block')
        expect(document.querySelector('.pay-cookie-settings-alert--success').style.display).not.toEqual('block')
      })
    })
    ...
    describe('after changing cookie settings - sets the back link in the success alert', () => {
      ...
      it('when there is no referrer, it does NOT show a back link', () => {
        ...
        expect(document.querySelector('.pay-cookie-settings__prev-page').style.display).toEqual('none')
      })

      it('when there is a referrer, it does show a back link', () => {
        Object.defineProperty(window.document, 'referrer', {
          value: 'http://www.example.org/page1',
          configurable: true,
        })
        window.GovUkPay.CookieSettings.submitSettingsForm(submitEvent)

        expect(document.querySelector('.pay-cookie-settings__prev-page').style.display).toEqual('block')

        expect(document.querySelector('.pay-cookie-settings__prev-page').href).toEqual('http://www.example.org/page1')
      })
