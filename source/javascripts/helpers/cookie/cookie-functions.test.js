...
describe('Cookie settings', () => {
  ...
  describe('setCookie', () => {
    ...
    it('sets allowed cookie with no options', () => {
      window.GovUkPay.Cookie.setCookie('govuk_pay_cookie_policy', '{"analytics":false}')

      expect(document.cookie).toEqual('govuk_pay_cookie_policy={"analytics":false}')
    })

    it('sets allowed cookie with expiry date as an option', () => {
      const cookieExpiryDays = 10
      const cookieString = window.GovUkPay.Cookie.setCookie('govuk_pay_cookie_policy', '{"analytics":false}', {
      ...
      const expiryDateString = cookieString.substring(cookieString.indexOf(searchString) + searchString.length)

      var expiryDate = new Date(expiryDateString)

      var previousDay = new Date(new Date().getTime() + 1000 * 60 * 60 * 24 * (cookieExpiryDays - 1))
      var nextDay = new Date(new Date().getTime() + 1000 * 60 * 60 * 24 * (cookieExpiryDays + 1))
  ...
  describe('setConsentCookie', () => {
    it('changes existing cookie value to false', () => {
      ...
      expect(window.GovUkPay.Cookie.getCookie('govuk_pay_cookie_policy')).toEqual('{"analytics":false}')
    })
    ...
    it('changes existing cookie value to true', () => {
      ...
      expect(window.GovUkPay.Cookie.getCookie('govuk_pay_cookie_policy')).toEqual('{"analytics":true}')
    })
  })
